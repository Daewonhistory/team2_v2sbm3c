...
CREATE TABLE RESTAURANT(
  ...
  FOREIGN KEY (reserveno) REFERENCES RESERVE (reserveno)
);
...
COMMENT ON COLUMN RESTAURANT.reserveno is '예약번호';
...
COMMENT ON TABLE REVIEWIMAGE is '리뷰이미지';
COMMENT ON COLUMN REVIEWIMAGE.rest_imgno is '레스토랑이미지번호';
COMMENT ON COLUMN REVIEWIMAGE.imagefile is '이미지파일명';
COMMENT ON COLUMN REVIEWIMAGE.thumbfile is '섬네일파일명';
COMMENT ON COLUMN REVIEWIMAGE.restno is '식당번호';
