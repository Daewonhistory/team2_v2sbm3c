...
public class AdmitPersonCont {
  ...
  public String update(Model model, HttpSession session, int admitpersonno) {
    ...
    if(accessType != null) {    // 관리자가 아닌경우
      if(accessType.equals("owner")) {  // 사장인경우
        int ownerno = (int) session.getAttribute("ownerno");
        int restnoOfOwner = this.restaurantProc.foreign(ownerno);
        if(restnoOfOwner != admitPersonVO.getRestno()) {    // 수정할 허용인원의 식당사장이 아닐경우
          System.out.println("not allowed user access");
