...
public class ScheduleCont {
	...
	public String create(Model model, HttpSession session, @RequestParam(defaultValue = "0") int restno) {
	  ...
	  if(accessType == null) { // 관리자 접속
		...
		restno = this.restaurantProc.findByOwnerR(accessOwnerno).get(0).getRestno();
		  
		int ownernoOfRestaurant = this.restaurantProc.read(restno).getOwnerno();  
			
		// 현재 식당의 사장이 아니라면 되돌려보냄
		if(accessOwnerno != ownernoOfRestaurant) {
	...
	@GetMapping("/update")
	public String update(Model model, HttpSession session, @RequestParam(defaultValue = "0") int restno) {
	  
	  String accessType = (String) session.getAttribute("type");
	  if(accessType == null) { // 관리자 접속
	    ArrayList<ScheduleVO> scheduleList = this.scheduleProc.list_by_restno(restno);
	    if(scheduleList.size() == 0) {  //스케줄이 생성되지 않은 경우 스케줄 등록페이지로
	      System.out.println("스케줄 미생성 레스토랑");
