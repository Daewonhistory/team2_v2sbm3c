...
public class ReviewProc implements ReviewProcInter {
    ...
    public ArrayList<ReviewDTO> list_paging(int now_page, int record_per_page) {
      ...
      */
      ...
      ArrayList<ReviewDTO> list = this.reviewDAO.list_paging(map);
    ...
    @Override
    public int foreign(int restno, int custno) {
      
      return this.reviewDAO.foreign(restno, custno);
    }
    ...
    public String pagingBox(int now_page, String list_file, int total_count, int record_per_page, int page_per_block) {
      ...
      if (now_grp >= 2) { // 현재 그룹번호가 2이상이면 페이지수가 11페이지 이상임으로 이전 그룹으로 갈수 있는 링크 생성
        str.append("<li class='page-item'><A class='page-link' href='" + list_file + "?now_page=" + _now_page );
      }
      ...
      for (int i = start_page; i <= end_page; i++) {
        ...
        if (now_page == i) { // 목록에 출력하는 페이지가 현재페이지와 같다면 CSS 강조(차별을 둠)
          ...
          str.append("<li class='page-item'><a class='page-link' href='" + list_file +  "?now_page=" + i + "'>" + i + "</a></li>");
        }
      }
      ...
      if (now_grp < total_grp) {
        str.append("<li class='page-item'><A class='page-link' href='" + list_file +  "?now_page=" + _now_page + "'>다음</A></span>");
      }
      str.append("</ul>");
      str.append("</nav>");
